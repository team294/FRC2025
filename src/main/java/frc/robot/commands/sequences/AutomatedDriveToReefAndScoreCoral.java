...
public class AutomatedDriveToReefAndScoreCoral extends SequentialCommandGroup {
  ...
  static final Map<ReefLevel, ElevatorWristPosition> reefToElevatorMap =
      new EnumMap<>(ReefLevel.class);
  ...
  /**
   * Drives to nearest reef position, preemptively moving the elevator to L2 if a piece is held, and
   * scores coral in given level. If not scoring on L4, the robot will drive fully up against the
   * reef, and then back up after scoring. If scoring on L4, the robot will score while not fully up
   * against the reef, and the routine ends when the piece is scored.
   *
   ...
   */
  public AutomatedDriveToReefAndScoreCoral(
      ReefLevel level,
        ...
        either(
            sequence(
                parallel(
                        sequence(
                            ...
                            either(
                                parallel(
                                    ...
                                    new DriveToReefWithOdometryForCoral(
                                        level, driveTrain, field, rightJoystick),
                                    sequence(
                                        deadline(
                                            waitSeconds(0.4),
                                            sequence(
                                                ...
                                                new WristElevatorSafeMove(
                                                    ElevatorWristPosition.CORAL_L1,
                                                    RegionType.CORAL_ONLY,
                                                    elevator,
                                                    wrist))),
                                        // Move elevator/wrist to correct position based on given
                                        // level
                                        new CoralScorePrepSequence(
                                            reefToElevatorMap.get(level),
                                            elevator,
                                            wrist,
                                            algaeGrabber,
                                            coralEffector))),
                                sequence(
                                    ...
                                    deadline(
                                        ...
                                        sequence(
                                            either(
                                                sequence(
                                                    new CoralIntakeSequence(
                                                        elevator, wrist, hopper, coralEffector),
                                                    runOnce(
                                                        () ->
                                                            LEDEventUtil.sendEvent(
                                                                LEDEventUtil.StripEvents
                                            ...
                                            deadline(
                                                    ...
                                                    new WaitUntilCommand(
                                                        () ->
                                                            (driveTrain
                                                                    ...
                                                                    .minus(
                                                                        field
                                                                            .getNearestReefScoringPositionWithOffset(
                                                                                driveTrain
                                                                                    .getPose(),
                                                                                new Transform2d(
                                                                                    (-RobotDimensions
                                                    ...
                                                    new WaitUntilCommand(
                                                        () ->
                                                            (driveTrain
                                                                    ...
                                                                    .minus(
                                                                        field
                                                                            .getNearestReefScoringPositionWithOffsetL1(
                                                                                driveTrain
                                                                                    .getPose(),
                                                                                new Transform2d(
                                                                                    (-RobotDimensions
                                                ...
                                                new WristElevatorSafeMove(
                                                    level == ReefLevel.L1
                                                        ? ElevatorWristPosition.CORAL_L1
                                                        : ElevatorWristPosition.CORAL_L2,
                                                    RegionType.CORAL_ONLY,
                                                    elevator,
                                                    wrist)),
                                            new CoralScorePrepSequence(
                            ...
                            // either(
                            //   new DriveToPose(CoordType.kRelative, () -> new
                            // Pose2d(DriveConstants.distanceFromReefToScore, 0, new Rotation2d(0)),
                            //       0.5, 1.0,
                            ...
                            either(
                                new DriveToPose(
                                    ...
                                    () ->
                                        new Pose2d(
                                            -DriveConstants.distanceFromReefToScore,
                                            0,
                                            Rotation2d.kZero),
                                    0.5,
                                    1.0,
                        ...
                        runOnce(
                            () ->
                                LEDEventUtil.sendEvent(
                                    LEDEventUtil.StripEvents.AUTOMATED_DRIVING_REEF)))
                    .handleInterrupt(
                        () ->
                            either(
                                runOnce(
                                    () ->
                                        LEDEventUtil.sendEvent(
                                            LEDEventUtil.StripEvents.CORAL_MODE)),
                                runOnce(
            ...
            () ->
                !(algaeGrabber.isAlgaePresent()
                    && reefToElevatorMap.get(level) == ElevatorWristPosition.CORAL_L4)));
  }

  /**
   * AutomatedDriveToReefAndScoreCoral for autonomous only If not scoring on L4, the robot will
   * drive fully up against the reef, and then back up after scoring. If scoring on L4, the robot
   * will score while not fully up against the reef, and the routine ends when the piece is scored.
   *
        ...
        parallel(
            sequence(
                ...
                deadline(
                    ...
                    // If the endeffector is in hold mode (Coral is safely in intake) then do
                    // nothing, otherwise intake the coral until the piece is present
                    sequence(
                        either(
                            none(),
                            ...
                            new WaitUntilCommand(
                                () ->
                                    (driveTrain
                                            .getPose()
                                            .minus(
                                                field.getReefScoringPositionWithOffset(
                                                    location,
                                                    new Transform2d(
                                            ...
                                            .getTranslation()
                                            .getNorm()
                                        <= DriveConstants.distanceFromReefToElevate)),
                            new WristElevatorSafeMove(
                                level == ReefLevel.L1
                                    ? ElevatorWristPosition.CORAL_L1
                                    : ElevatorWristPosition.CORAL_L2,
                ...
                // Move elevator/wrist to correct position based on given level, only if isLastCoral
                // is not true (meaning we are not at the last coral so we still want to move
                // elevator)
                either(
                    new CoralScorePrepSequence(
                        reefToElevatorMap.get(level), elevator, wrist, algaeGrabber, coralEffector),
                    none(),
                    () -> !score),
                either( // Runs this whole code section if isLastCoral is not true and we actually
                    // want to move elevator and score
                    sequence(
                        // If scoring on L1, drive forward to get to the reef
                        // either(
                        //   new DriveToPose(CoordType.kRelative, () -> new
                        ...
                        //     waitSeconds(0.5),
                        //     new DriveToPose(CoordType.kRelative, () -> new
                        // Pose2d(-DriveConstants.distanceFromReefToScore, 0, Rotation2d.kZero),
                        //         0.5, 1.0,
                        //         TrajectoryConstants.maxPositionErrorMeters,
                        // TrajectoryConstants.maxThetaErrorDegrees,
                        //         true, true, driveTrain)
