...
public class ChoreoFollower extends Command {
  ...
  private final DoubleLogEntry dLogTrajRotVel =
      new DoubleLogEntry(log, "/DrivePathFollower/trajRotVel");
  ...
  private final DoubleLogEntry dLogRobotRotVel =
      new DoubleLogEntry(log, "/DrivePathFollower/robotRotVel");

  /**
   * Choreo follower used to follow Choreo trajectories. This method takes in a choreo trajectory
   * and follows the contents of that trajectory until the trajectory is finished.
   *
   * <p><b>NOTE:</b> This follower will *not* set the outputVolts to zero upon completion of the
   * path. This is left to the user to do since it is not appropriate for paths with nonstationary
   * endstates.
   *
   ...
   * @param outputChassisSpeeds a function that takes in a field-relative chassis speeds and
   *     controls the robot
   * @param poseSupplier a pose supplier to get the current robot position
   * @param mirrorTrajectory determines if the trajectory will be mirrored: when true the trajectory
   *     will mirror while internal robot odometry will remain the same, when false the trajectory
   *     will not be changed
   * @param driveTrain DriveTrain subsystem
   */
  public ChoreoFollower(
    ...
    dLogRobotVel.append(
        Math.hypot(robotSpeeds.vxMetersPerSecond, robotSpeeds.vyMetersPerSecond), timeNow);
  ...
  public void execute() {
    ...
    Optional<SwerveSample> sampleNextOpt =
        trajectory.sampleAt(curTime + Constants.SwerveConstants.dt, mirrorTrajectoryThisInit);
    ...
    if (!sampleCurrOpt.isEmpty() && !sampleNextOpt.isEmpty()) {
      ...
      rotationFeedback =
          rotationController.calculate(poseCurr.getRotation().getRadians(), sampleCurr.heading);
    ...
    return (timer.hasElapsed(trajectory.getTotalTime())
            ...
            && Math.abs(
                    MathBCR.normalizeAngle(
                        poseCurr.getRotation().getDegrees() - finalPose.getRotation().getDegrees()))
                <= Constants.TrajectoryConstants.choreoMaxThetaErrorDegrees
            && Math.hypot(
                    driveTrain.getChassisSpeeds().vxMetersPerSecond,
                    driveTrain.getChassisSpeeds().vyMetersPerSecond)
                <= Constants.TrajectoryConstants.endVelocityErrorDegrees)
