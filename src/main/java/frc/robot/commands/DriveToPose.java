...
public class DriveToPose extends Command {
  ...
  private Pose2d initialPose,
      goalPose; // Starting and destination robot pose (location and rotation) on the field
  private Translation2d initialTranslation; // Starting robot translation on the field
  private Translation2d
      goalDirection; // Unit vector pointing from initial pose to goal pose = direction of travel
  private TrapezoidProfileBCR
      profile; // Relative linear distance/speeds from initial pose to goal pose

  private Translation2d
      curRobotTranslation; // Current robot translation relative to initialTranslation

  private SendableChooser<Integer> feedbackChooser =
      new SendableChooser<>(); // Shuffleboard chooser for turning parameters on/off for tuning the
  // drive base

  private static final int FEEDBACK_NORMAL = 0; // Use position and velocity feedback
  private static final int FEEDBACK_VELOCITY_ONLY =
      1; // Use velocity feedback only, do not use position feedback
  private static final int FEEDBACK_NONE = 2; // Turn off velocity and position feedback

  private final DataLog log = DataLogManager.getLog();
  private final StringLogEntry dLogTrajType =
      new StringLogEntry(log, "/DrivePathFollower/trajType");
  private final StructLogEntry<Pose2d> dLogCurPose2D =
      StructLogEntry.create(log, "/DrivePathFollower/curPose2d", Pose2d.struct);
  private final StructLogEntry<Pose2d> dLogTrajPose2D =
      StructLogEntry.create(log, "/DrivePathFollower/trajPose2d", Pose2d.struct);
  ...
  private final DoubleLogEntry dLogTrajAccel =
      new DoubleLogEntry(log, "/DrivePathFollower/trajAccel");
  private final DoubleLogEntry dLogTrajVel = new DoubleLogEntry(log, "/DrivePathFollower/trajVel");
  private final DoubleLogEntry dLogTrajVelkA =
      new DoubleLogEntry(log, "/DrivePathFollower/trajVelkA");
  private final DoubleLogEntry dLogTrajVelAng =
      new DoubleLogEntry(log, "/DrivePathFollower/trajVelAng");
  private final DoubleLogEntry dLogTrajRot = new DoubleLogEntry(log, "/DrivePathFollower/trajRot");
  private final DoubleLogEntry dLogRobotPosErr =
      new DoubleLogEntry(log, "/DrivePathFollower/robotPosErr");
  private final DoubleLogEntry dLogRobotThErr =
      new DoubleLogEntry(log, "/DrivePathFollower/robotThErr");
  ...
  private final DoubleLogEntry dLogRobotVel =
      new DoubleLogEntry(log, "/DrivePathFollower/robotVel");
  private final DoubleLogEntry dLogRobotXVel =
      new DoubleLogEntry(log, "/DrivePathFollower/robotXVel");
  private final DoubleLogEntry dLogRobotYVel =
      new DoubleLogEntry(log, "/DrivePathFollower/robotYVel");
  private final DoubleLogEntry dLogRobotVelAng =
      new DoubleLogEntry(log, "/DrivePathFollower/robotVelAng");
  private final DoubleLogEntry dLogRobotRot =
      new DoubleLogEntry(log, "/DrivePathFollower/robotRot");
  private final DoubleLogEntry dLogRobotPitch =
      new DoubleLogEntry(log, "/DrivePathFollower/robotPitch");

  /**
   * Drives the robot to the desired pose in field-relative or robot-relative coordinates. Stops the
   * robot at the end of the command, unless the command is interrupted.
   *
   * @param poseType Determines how goalPose is interpreted:
   *     <p>kRelative = goalPose is in the robot coordinate system
   *     <p>kAbsolute = goalPose is in the Blue field coordinate system, based on the Blue alliance.
   *     The goal pose is unaffected by the currently selected alliance (red or blue).
   ...
   *     <p>Robot forward movement, in meters (0 = current robot location, + = robot forward, - =
   *     robot reverse)
   *     <p>Robot left movement, in meters (0 = current robot location, + = robot left, - = robot
   *     right)
   *     <p>Robot angle, in degrees (0 = facing away from the Blue drivestation, + to the left, - to
   *     the right)
   *     <p>For poseType = kAbsolute:
   *     <p>Desired X location in the field, in meters (0 = field edge in front of the Blue driver
   *     station, + = away from the Blue drivestation)
   *     <p>Desired Y location in the field, in meters (0 = right edge of field when standing in the
   *     Blue driver station, + = left when looking from the Blue drivestation)
   *     <p>Desired angle on the field, in degrees (0 = facing away from the Blue drivestation, + to
   ...
   * @param maxAccelMetersPerSecondSquare max acceleration/deceleration, in meters per second
   *     squared
   ...
   * @param closedLoopSwerve true = use velocity control feedback+feedforward on swerve motors
   *     (normal), false = use feed-forward only on swerve motors (test mode)
   * @param usePositionFeedback true = use position feedback to keep robot movement accuracy
   *     (normal), false = turn off postion feedback (test mode)
   * @param driveTrain DriveTrain subsystem
   */
  public DriveToPose(
      CoordType poseType,
      Supplier<Pose2d> goalPoseSupplier,
      double maxVelMetersPerSecond,
      double maxAccelMetersPerSecondSquare,
      double maxPositionErrorMeters,
      double maxThetaErrorDegrees,
      boolean closedLoopSwerve,
      boolean usePositionFeedback,
    ...
    trapProfileConstraints =
        new TrapezoidProfileBCR.Constraints(
            MathUtil.clamp(
                maxVelMetersPerSecond,
                -SwerveConstants.kFullSpeedMetersPerSecond,
                SwerveConstants.kFullSpeedMetersPerSecond),
            MathUtil.clamp(
                maxAccelMetersPerSecondSquare,
                -SwerveConstants.kFullAccelerationMetersPerSecondSquare,
                SwerveConstants.kFullAccelerationMetersPerSecondSquare));

    constructorCommonCode();

    // Contoller object is created in constructorCommonCode(), so setting the controller
    // enabled/disabled has to occur afterwards
   ...
   *
   * @param type CoordType, kRelative (turn relative to current robot angle) or kAbsolute (turn to
   *     Blue field angle). kAbsoluteResetPose, kAbsoluteResetPoseTol = Don't use (behaves same as
   *     kAbsolute).
   * @param rotation rotation to turn to, in degrees (+=turn left, -=turn right). For absolute
   *     rotation, the 0 degrees is facing away from the Blue driver station.
  ...
  /**
   * Drives the robot to the desired pose based on numbers inputed in shuffleboard. Stops the robot
   * at the end of the command, unless the command is interrupted.
   *
   * @param poseType Determines how goalPose is interpreted:
   *     <p>kRelative = goalPose is in the robot coordinate system
   *     <p>kAbsolute = goalPose is in the Blue field coordinate system, based on the Blue alliance.
  ...
  private void constructorCommonCode() {
    ...
    dLogRobotVel.append(
        Math.hypot(robotSpeeds.vyMetersPerSecond, robotSpeeds.vxMetersPerSecond), timeNow);
    dLogRobotXVel.append(robotSpeeds.vxMetersPerSecond, timeNow);
    dLogRobotYVel.append(robotSpeeds.vyMetersPerSecond, timeNow);
    dLogRobotVelAng.append(
        Math.toDegrees(Math.atan2(robotSpeeds.vyMetersPerSecond, robotSpeeds.vxMetersPerSecond)),
        timeNow);
  ...
  public void initialize() {
    ...
    switch (goalMode) {
      ...
      case shuffleboard:
        ...
        Rotation2d angleTarget =
            Rotation2d.fromDegrees(SmartDashboard.getNumber("DriveToPose Rot degrees", 0));
        inputPose = new Pose2d(xPos, yPos, angleTarget);

        trapProfileConstraints.maxVelocity =
            MathUtil.clamp(
                SmartDashboard.getNumber("DriveToPose VelMax mps", 0),
                -SwerveConstants.kFullSpeedMetersPerSecond,
                SwerveConstants.kFullSpeedMetersPerSecond);
      ...
      if (poseType == CoordType.kRelative) {
        goalPose =
            driveTrain
                .getPose()
                .plus(
                    (new Transform2d(inputPose.getX(), inputPose.getY(), inputPose.getRotation())));
      } else {
        goalPose = inputPose;
      }
    ...
    double initialVelocity =
        robotSpeed.vxMetersPerSecond * goalDirection.getX()
            + robotSpeed.vyMetersPerSecond * goalDirection.getY();

    // Create the profile, which is linear distance (along goalDirection) relative to the initial
    // pose
  ...
  public void execute() {
    ...
    // Current robot location, translation is relative to starting position, rotation is absolute
    // field rotation
    ...
    Pose2d robotPose =
        new Pose2d(curRobotTranslation, Rotation2d.fromDegrees(driveTrain.getPoseAngle()));

    // Calculate current desired pose and velocity from the Trapezoid profile, relative to starting
    // position
    TrapezoidProfileBCR.State desiredState = profile.calculate(curTime);
    Pose2d desiredPose =
        new Pose2d(goalDirection.times(desiredState.position), goalDirection.getAngle());

    // Fudge in some kA
    double desiredVelocityMetersPerSecond =
        desiredState.velocity + (desiredState.acceleration * SwerveConstants.kADriveToPose);
    ...
    ChassisSpeeds targetChassisSpeeds =
        controller.calculate(
            robotPose, desiredPose, desiredVelocityMetersPerSecond, desiredRotation);
    ...
    dLogTrajPose2D.append(
        new Pose2d(desiredPose.getTranslation().plus(initialTranslation), desiredRotation),
        timeNow); // Field-relative current desired position
    ...
    dLogRobotPosErr.append(
        driveTrain.getPose().getTranslation().minus(goalPose.getTranslation()).getNorm(), timeNow);
    dLogRobotThErr.append(
        MathBCR.angleMinus(driveTrain.getPoseAngle(), goalPose.getRotation().getDegrees()),
        timeNow);
  ...
  public boolean isFinished() {
    ...
    if (finished) {
      DataLogUtil.writeMessage(
          "DriveToPose: Finished, angleError =",
          angleError,
          "posError",
          posError,
          "maxTheta =",
