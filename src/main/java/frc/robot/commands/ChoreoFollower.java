...
public class ChoreoFollower extends Command {
   ...
   *
   * <p>Note: This follower will *not* set the outputVolts to zero upon completion of the path.
   ...
   *                            will mirror while internal robot odometry will remain the same.
  ...
  public void initialize() {
    ...
    log.writeLog(false, "ChoreoFollower", "Init", 
      "Is Flipped", mirrorTrajectoryThisInit);
  ...
  public void end(boolean interrupted) {
    ...
    log.writeLogEcho(true, "ChoreoFollower", "End");
  }
  ...
  public boolean isFinished() {
    ...
    if(finalPoseOpt.isEmpty()){
      log.writeLogEcho(true, "ChoreoFollower", "IsFinished", "No final Pose exists");
      return true;
    }
    ...
    return (timer.hasElapsed(trajectory.getTotalTime())
      ...
      && Math.abs(MathBCR.normalizeAngle(poseCurr.getRotation().getDegrees() - finalPose.getRotation().getDegrees())) <= Constants.TrajectoryConstants.choreoMaxThetaErrorDegrees 
      && Math.hypot(driveTrain.getChassisSpeeds().vxMetersPerSecond, driveTrain.getChassisSpeeds().vyMetersPerSecond) <= Constants.TrajectoryConstants.endVelocityErrorDegrees) ||
