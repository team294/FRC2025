...
public class AutoDriveToReef extends SequentialCommandGroup {
  /**
   * Below constructor is outdated and not used anywhere Drives to reef location (using trajectory)
   * and prepares elevator based on the reef level. Runs intake command while driving to ensure
   * coral is fully in, and then move the elevator after done driving.
   *
  ...
  /**
   * Drives to reef location, <b> trajectory gets cut off early (commented out) </b> to allow for
   * smooth transition into AutomatedDriveToReefAndScoreCoral (does not move elevator) Runs intake
   * command while driving to ensure coral is fully in, and then move the elevator after done
   * driving.
   *
  ...
  public AutoDriveToReef(
    ...
    Trajectory<SwerveSample> trajectory =
        fromHP ? AutoSelection.getHPToReef(end) : AutoSelection.getBargeToReef(end);
    addCommands(
        ...
        deadline(
            ...
            // Timeout in case coral doesn't make it into hopper.
            new DriveTrajectory(
                    CoordType.kAbsolute, StopType.kBrake, trajectory, driveTrain, alliance)
                .withTimeout(trajectory.getTotalTime() - 0.5),
            sequence(
                new CoralIntakeSequence(elevator, wrist, hopper, coralEffector),
                new WristElevatorSafeMove(
                    ElevatorWristPosition.CORAL_L1, RegionType.CORAL_ONLY, elevator, wrist))),
        new DataLogMessage(false, "AutoDriveToReef: End, trajectory =", trajectory.name())

        // new WristElevatorSafeMove(reefToElevatorMap.get(level), RegionType.CORAL_ONLY, elevator,
        // wrist, log)

        // Drive to the given reef position
        // new DriveToPose(CoordType.kAbsolute, () -> (field.getRobotReefScoringPosition(end)), 0.5,
        // 1.0, TrajectoryConstants.maxPositionErrorMeters,
        // TrajectoryConstants.maxThetaErrorDegrees, true, true, driveTrain, log)
        );
  }
}
