...
public class AutoCoralCycleLoop extends SequentialCommandGroup {
  /**
   * First, score the pre-loaded coral. Then, do cycles of (1) drive to HP, (2) intake, (3) drive to
   * reef, and (4) score. TODO Assumes each adjacent reef location in the list have a valid
   * trajectory between them and HP TODO Figure out proper exit conditions and timeouts
   *
   * @param reefLocations list of ReefLocation to visit, in order
   * @param reefLevels list of ReefLevel to score on, in order
   * @param scoreLastCoral true = we want to move the elevator to score the coral false = stop at
   *     reef DOESN'T WORK
  ...
  public AutoCoralCycleLoop(
    ...
    if (reefLocations == null || reefLocations.size() == 0) {
              ...
              new InstantCommand(
                  () ->
                      coralEffector.setCoralEffectorPosition(
                          coralEffector.getCoralEffectorPosition(), true)),
              none(),
      ...
      // After scoring the pre-load, do cycles of (1) drive to HP, (2) intake, (3) drive to reef,
      // and (4) score
        ...
        for (int i = 0; i < reefLocations.size() - 1; i++) {
          ...
          boolean isLastCoral =
              !scoreLastCoral
                  && reefLocations.get(i)
                      == reefLocations.get(
                          reefLocations.size() - 1); // TODO this boolean does not work

          addCommands(
              // If it is the final coral location, then isFinalCoral = true and the elevator does
              // not move up. If not, then the elevator moves up to score
              new AutoCoralCycle(
                  start,
                  end,
                  isLastCoral,
      ...
      // Drive back to and end at HP after the last reef location if we are to end at HP. Otherwise,
      // we are ending at reef
      if (endAtHP && reefLocations.size() > 0) {
        addCommands(
            new AutoCoralDriveAndIntakeSequence(
                reefLocations.get(reefLocations.size() - 1),
                driveTrain,
                elevator,
                wrist,
                coralEffector,
