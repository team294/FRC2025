...
public class ElevatorProfileGenerator {
  ...
  private double
      currentMPDistance; // Distance that should have been traveled in the current motion profile
  // (always positive)
  private double
      targetMPDistance; // Total distance to be traveled in the current motion profile (always
  // positive)
  ...
  private double maxAcceleration =
      600.0; // Max acceleration, in inches/second^2 CALIBRATED  4/18:  Was 180.0, now 300.   6/4:
  // Was 300, now 600.  (turned off ramp rate limit)
  private double stoppingAcceleration =
      0.60 * maxAcceleration; // Limit the stopping acceleration to 75% of the max acceleration
  // 4/18:  Was 0.75, dropped to 0.60 when maxAcceleration was increased
  // from 180 to 300.
  private double
      currentMPAcceleration; // Acceleration that it should be at in the current motion profile
  private boolean approachingTarget =
      false; // true = close enough to target to be decelerating, false = not close enough

  private double MPdt =
      0.02; // delta T (time) in seconds forecasted between future calls to updateProfileCalcs
  private double
      measuredDt; // delta T (time) in seconds measured between this call and the prior call to
  // updateProfileCalcs

  private double directionSign; // +1 if finalPosition > initialPosition, -1 if not

  private long startTime, lastTime;
  private double prevError, error, intError, lastMPVelocity;
  ...
  private double kAu =
      0.003
          / ElevatorConstants
              .compensationVoltage; // In (pct-output)/(in/s^2)    4/18:  Was 0, now 0.008.   9/21:
  // Was 0.008, now 0.003 (turned off ramp rate limit)
  private double kPu =
      0.05; // In (pct-output)/(in)  4/18: was 0.10, now 0.08 to prevent elevator overshoots on
  // short movements (like L1 to L2).  6/4:  Was 0.08, now 0.05 [maybe increase?] (turned
  // off ramp rate limit)
  ...
  private final DoubleLogEntry dLogTargetPos =
      new DoubleLogEntry(log, "/ElevatorProfile/TargetPos");
  ...
  private final DoubleLogEntry dLogActualPos =
      new DoubleLogEntry(log, "/ElevatorProfile/ActualPos");
  private final DoubleLogEntry dLogMPVel = new DoubleLogEntry(log, "/ElevatorProfile/MPVel");
  private final DoubleLogEntry dLogActualVel =
      new DoubleLogEntry(log, "/ElevatorProfile/ActualVel");
   ...
   * to take control of the elevator, so only call it if the encoders are working (check interlocks
   * first).
   *
  ...
  public void setProfileTarget(double pos) {
    ...
    DataLogUtil.writeMessage(
        "ElevatorProfile:  New Profile, InitPos =", initialPosition, ", Target =", finalPosition);
  }

  /**
   * Call this method once per scheduler cycle. This method calculates the distance that the
   * elevator should have traveled at this point in time, per the motion profile. It also calculates
  ...
  public void updateProfileCalcs() {
    ...
    if (currentMPDistance < targetMPDistance) {
      ...
      // The profile *should be* close enough to the target to be decelerating, so update the
      // acceleratioin
      if (currentMPVelocity >= 0
          && (approachingTarget || (targetMPDistance - currentMPDistance) < stoppingDistance)) {
        approachingTarget = true;
        // If the elevator is too close to the target, then it may need to decelerate faster than
        // stoppingAcceleration.
        // If not, then just use stoppingAcceleration.
        currentMPAcceleration =
            Math.min(
                -stoppingAcceleration,
                -0.5
                    * currentMPVelocity
                    * currentMPVelocity
                    / (targetMPDistance - currentMPDistance));
      }
      ...
      if (approachingTarget && currentMPVelocity < 0.0) {
        // Do not set the acceleration too negative, since that would cause velocity to switch
        // direction later in these calculations
      ...
      // The profile has reached its target, so do not change the theoretical distance and zero the
      // profile velocity and acceleration
    } else {
      currentMPDistance = targetMPDistance;
      lastMPVelocity = 0;
      currentMPVelocity = 0;
  ...
  /**
   * Makes the elevator follow the motion profile. This method should be called exactly once per
   * scheduler cycle. NOTE: Only follow the motion if elevPosControl is true. Otherwise, it is in
   * manual control mode, so do nothing (return 0.0).
   *
  ...
  public double trackProfilePeriodic() {
    ...
    if (directionSign == 1) {
      // percentPowerFF = kFF + kSu * Math.signum(currentMPVelocity * directionSign) + kVu *
      // currentMPVelocity * directionSign + kAu * currentMPAcceleration * directionSign;
      percentPowerFF =
          kFF
              + kSu * Math.signum((currentMPVelocity + lastMPVelocity) * directionSign)
              + Bdu * directionSign * (currentMPVelocity - Adu * lastMPVelocity);
      ...
      // percentPowerFF = kFF + kSd * Math.signum(currentMPVelocity * directionSign) + kVd *
      // currentMPVelocity * directionSign + kAd * currentMPAcceleration * directionSign;
      percentPowerFF =
          kFF
              + kSd * Math.signum((currentMPVelocity + lastMPVelocity) * directionSign)
              + Bdd * directionSign * (currentMPVelocity - Add * lastMPVelocity);
      percentPowerFB = kPd * error + ((error - prevError) * kDd) + (kId * intError);

      // If the elevator target = the initial position (i.e. directionSign = 0), then just use
      // feedback
    } else {
      percentPowerFB = kPu * error + ((error - prevError) * kDu) + (kIu * intError);
   ...
   *
   * @param logWhenDisabled true = write when robot is enabled or disabled, false = only write when
   *     robot is enabled
   */
