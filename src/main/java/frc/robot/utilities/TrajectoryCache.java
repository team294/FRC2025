...
/**
 * Class that caches all trajectories that the robot could run. Create one object instance of this
 * class when the robot initializes to build the trajectories.
 */
public class TrajectoryCache {
  ...
  public enum TrajectoryName {
    ...
    TrajectoryName(String trajFileName) {
      this.trajFileName = trajFileName;
    }
  }

  private static int trajectoryCount = TrajectoryName.values().length;
  private HashMap<TrajectoryName, Trajectory<SwerveSample>> cache =
      new HashMap<TrajectoryName, Trajectory<SwerveSample>>(
          trajectoryCount); // HashMap of Choreo Trajectories
  ...
  /**
   * Loads the choreo trajectory into the trajectory cache that has the same name as the
   * trajectoryType enum's trajFileName String If a trajectory with that String is unable to be
   * loaded a sticky fault will be recorded and no trajectory will be added to the trajectory cache
   *
   * @param trajectoryLoad the trajectoryName enum that will be loaded into the trajectory cache
   *     with the enum as the key
   */
  public void cacheTrajectory(TrajectoryName trajectoryLoad) {
    ...
    if (traj.isPresent()) {
      ...
      RobotPreferences.recordStickyFaults(
          "Trajectory-CacheTrajectory-" + trajectoryLoad.trajFileName + "-not-loaded");
    }
  }
   ...
   * @return the Choreo trajectory in the trajectory cache. Null if the requested trajectory was not
   *     loaded successfully.
   */
   ...
   */
  public static Trajectory<SwerveSample> mirrorVertically(
      Trajectory<SwerveSample> originalTraj, String newTrajName) {
    var flippedStates = new ArrayList<SwerveSample>();
    for (var state : originalTraj.samples()) {
      flippedStates.add(
          mirrorSampleVertically(state)); // flip all of the data of each swerve sample
    }
    return new Trajectory<SwerveSample>(
        newTrajName, flippedStates, originalTraj.splits(), originalTraj.events());
  }
