...
public class SwerveModule {
  ...
  private final StatusSignal<Voltage>
      driveMotorSupplyVoltage; // Incoming bus voltage to motor controller, in volts
  ...
  private final StatusSignal<Current>
      driveStatorCurrent; // Motor stator current, in amps (positive = forward, negative = reverse)
  ...
  private final StatusSignal<AngularVelocity>
      driveEncoderVelocity; // Encoder velocity, in pinion rotations/second
  ...
  private final StatusSignal<Current>
      turningStatorCurrent; // Motor stator current, in amps (positive = forward, negative =
  // reverse)
  private final StatusSignal<Angle> turningEncoderPosition; // Encoder position, in pinion rotations
  private final StatusSignal<AngularVelocity>
      turningEncoderVelocity; // Encoder velocity, in pinion rotations/second

  // Variables to track motor information
  private boolean
      isInCoastMode; // Current NeutralMode setting for the drive and turning motors, with true =
  // coast mode, false = brake mode
  ...
  private final StatusSignal<Angle>
      turningCanCoderPosition; // CANcoder position, in CANcoder rotations
  private final StatusSignal<AngularVelocity>
      turningCanCoderVelocity; // CANcoder velocity, in CANcoder rotations/second

  // Variables for encoder zeroing
  private double driveEncoderZero =
      0; // Reference raw encoder reading for drive motor encoder. Calibration sets this to zero.
  private double cancoderZero =
      0; // Reference raw encoder reading for CanCoder. Calibration sets this to the absolute
  // position from RobotPreferences.
  private double turningEncoderZero =
      0; // Reference raw encoder reading for turning motor encoder. Calibration sets this to match
  // the CanCoder.
  ...
  private boolean inVelocityMode =
      true; // true = last drive request was velocity control (or stopped), false = last drive
  // request was direct non-zero voltage control
   ...
   * @param turningOffsetDegrees Offset degrees in the turning motor to point to the front of the
   *     robot. Value is the desired encoder zero point, in absolute magnet position reading.
   * @param kVm Drive motor kV multiplier to account for small differences between the 4 swerve
   *     modules on the robot. The drive motor kV = kVDriveAvg * kVm.
   */
  public SwerveModule(
      String swName,
      int driveMotorAddress,
      int turningMotorAddress,
      int cancoderAddress,
    ...
    driveFeedforward =
        new SimpleMotorFeedforward(
            SwerveConstants.kSDrive, SwerveConstants.kVDriveAvg * kVm, SwerveConstants.kADrive);
    ...
    driveMotorConfig.MotorOutput.NeutralMode =
        NeutralModeValue.Brake; // Boot to brake mode, so robot starts quickly in auto
    ...
    driveMotorConfig.CurrentLimits.SupplyCurrentLimit =
        60.0; // Upper limit for the current, in amps
    driveMotorConfig.CurrentLimits.SupplyCurrentLowerLimit =
        35.0; // Lower limit for the current, in amps
    driveMotorConfig.CurrentLimits.SupplyCurrentLowerTime =
        2.0; // Threshold time, in seconds (was 0.1s, changed to 2.0s)
    ...
    driveMotorConfig.Slot0.kD =
        0.000012; // kD = (desired-output-volts) / (error-in-encoder-rps/sec)
    ...
    turningMotorConfig.MotorOutput.Inverted =
        turningMotorInverted
            ? InvertedValue.Clockwise_Positive
            : InvertedValue.CounterClockwise_Positive;
    turningMotorConfig.MotorOutput.NeutralMode =
        NeutralModeValue.Brake; // Boot to brake mode, so robot starts quickly in auto
    ...
    // limited to the lower limit.
    // This is configured to prevent the breakers from tripping.
    turningMotorConfig.CurrentLimits.SupplyCurrentLimit =
        60.0; // Upper limit for the current, in amps
    turningMotorConfig.CurrentLimits.SupplyCurrentLowerLimit =
        35.0; // Lower limit for the current, in amps
    ...
    turningMotorConfig.Slot0.kP =
        3.4; // kP = (desired-output-volts) / (error-in-encoder-rotations). ETU calibrated at 4.0
    // but reduced to 3.4 to minimize jitter TODO CALIBRATE FOR 2025
    turningMotorConfig.Slot0.kI = 0.0;
    turningMotorConfig.Slot0.kD =
        0.072; // kD = (desired-output-volts) / (error-in-encoder-rps) TODO CALIBRATE FOR 2025
    ...
    turningCanCoderConfig.MagnetSensor.AbsoluteSensorDiscontinuityPoint =
        0.5; // 1 makes absolute position unsigned [0, 1); 0.5 makes it signed [-0.5, 0.5), 0 makes
    // it always negative
    turningCanCoderConfig.MagnetSensor.SensorDirection =
        cancoderReversed
    ...
    setMotorModeCoast(
        false); // false on boot up, so robot starts quickly in auto. Set to false in autoInit or
    // teleopInit, but true when disabled.

    // Create logfile entries
    dLogCCAngle =
        new DoubleLogEntry(
            log, StringUtil.buildString("/DriveTrain/Swerve", swName, "/CCAngleDeg"));
    dLogCCTurnRate =
        new DoubleLogEntry(
            log, StringUtil.buildString("/DriveTrain/Swerve", swName, "/CCAngleRateDPS"));
    dLogFXAngle =
        new DoubleLogEntry(
            log, StringUtil.buildString("/DriveTrain/Swerve", swName, "/FXSngleDeg"));
    dLogFXTurnRate =
        new DoubleLogEntry(
            log, StringUtil.buildString("/DriveTrain/Swerve", swName, "/FXAngleRateDPS"));
    dLogTurnTemp =
        new DoubleLogEntry(log, StringUtil.buildString("/DriveTrain/Swerve", swName, "/TurnTemp"));
    dLogTurnOutput =
        new DoubleLogEntry(
            log, StringUtil.buildString("/DriveTrain/Swerve", swName, "/TurnOutput"));
    dLogDriveTemp =
        new DoubleLogEntry(log, StringUtil.buildString("/DriveTrain/Swerve", swName, "/DriveTemp"));
    dLogDriveOutput =
        new DoubleLogEntry(
            log, StringUtil.buildString("/DriveTrain/Swerve", swName, "/DriveOutput"));
    dLogDriveDist =
        new DoubleLogEntry(
            log, StringUtil.buildString("/DriveTrain/Swerve", swName, "/DriveDistMeters"));
    dLogDriveSpeed =
        new DoubleLogEntry(
            log, StringUtil.buildString("/DriveTrain/Swerve", swName, "/DriveSpeedMPS"));
  ...
  /**
   * Configures the motors and encoders, uses default values from the constructor. In general, this
   * should only be called from the constructor when the robot code is starting. However, if the
   * robot browns-out or otherwise partially resets, then this can be used to force the encoders to
   * have the right calibration and settings, especially the calibration angle for each swerve
   * module.
   *
  ...
  public void configSwerveModule() {
    ...
    // NOTE!!! When the Cancoder or TalonFX encoder settings are changed above, then the next call
    // to
    // getCanCoderDegrees() getTurningEncoderDegrees() may contain an old value, not the value based
    // on
    // the updated configuration settings above!!!!  The CANBus runs asynchronously from this code,
    // so
    // sending the updated configuration to the CanCoder/TalonFX and then receiving an updated
    // position
    ...
    // System.out.println(swName + " CanCoder " + getCanCoderDegrees() + " FX " +
    // getTurningEncoderDegrees() + " pre-CAN");
    zeroDriveEncoder();
    // DataLogUtil.writeMessageEcho("SwerveModule ", swName + " pre-CAN", ", Cancoder: ",
    // getCanCoderDegrees(), ", FX: ", getTurningEncoderDegrees());
    calibrateCanCoderDegrees(turningOffsetDegrees);
    // DataLogUtil.writeMessageEcho(true, "SwerveModule ", swName + " post-CAN", ", Cancoder: ",
    // getCanCoderDegrees(), ", FX: ", getTurningEncoderDegrees());
    calibrateTurningEncoderDegrees(getCanCoderDegrees());
    // DataLogUtil.writeMessageEcho(true, "SwerveModule ", swName + " post-FX", ", Cancoder: ",
    // getCanCoderDegrees(), ", FX: ", getTurningEncoderDegrees());
  }
   ...
   *
   * <p><b>NOTE:</b> this procedure includes multiple blocking calls and will delay robot code by
   * ~60ms if the mode is changed. However, if setCoast is the same as the current setting, then
   * nothing is sent to the swerve modules and there will not be a delay.
   *
  ...
  public void setTurnMotorPercentOutput(double percentOutput) {
    turningMotor.setControl(
        turningVoltageControl.withOutput(percentOutput * SwerveConstants.voltageCompSaturation));
  }

  /**
   * Sets the facing of the wheel (direction the wheel is pointing).
   *
   * <p><b>NOTE:</b> This sets the absolute facing. It does <b>not</b> turn the wheel to the
   * "optimized" facing +/-180 degrees if that is closer.
   *
   * @param angle desired wheel facing in degrees, -180 to +180 (positive = left, negative = right,
   *     0 = facing front of robot)
   */
  public void setWheelFacing(double angle) {
    angle = MathBCR.normalizeAngle(angle);
    turningMotor.setControl(
        turningPositionControl.withPosition(calculateTurningEncoderTargetRaw(angle)));
  }

  /**
   * Sets the desired state for the module, using closed loop controls on the Talons. The Talons
   * will hold this state until commanded to stop or another state.
   *
   * @param desiredState desired SwerveModuleState with speed and angle
   * @param isOpenLoop true = fixed drive percent output to approximate velocity, false = closed
   *     loop drive velocity control
   */
  public void setDesiredState(SwerveModuleState desiredState, boolean isOpenLoop) {
    ...
    // CTRE is not
    desiredState =
        MathSwerveModuleState.optimize(
            desiredState, Rotation2d.fromDegrees(getTurningEncoderDegrees()));

    double ff;
    if (inVelocityMode
        && Math.abs(priorDriveVelocity - desiredState.speedMetersPerSecond)
            >= SwerveConstants.velMinDeltaUsingkA) {
      ...
      // If actual speed is close to desired speed, then do not use kA (feedback with actual
      // velocity)
    ...
    turningMotor.setControl(
        turningPositionControl.withPosition(
            calculateTurningEncoderTargetRaw(desiredState.angle.getDegrees())));
  ...
  public void zeroDriveEncoder() {
    ...
    DataLogUtil.writeMessageEcho(
        buildString("SwerveModule ", swName),
        ": ZeroDriveEncoder, driveEncoderZero =",
        driveEncoderZero,
        ", raw encoder =",
        getDriveEncoderRotations(),
        ", encoder meters =",
        getDriveEncoderMeters());
  }
  ...
  public double getDriveEncoderMeters() {
    return (getDriveEncoderRotations() - driveEncoderZero)
        * SwerveConstants.kDriveEncoderMetersPerTick;
  }
   ...
   * @return drive motor raw value equivalent velocity, in encoder ticks (= pinion rotations) per
   *     second
   */
  ...
  public void calibrateTurningEncoderDegrees(double currentAngleDegrees) {
    turningEncoderZero =
        getTurningEncoderRaw()
            - (currentAngleDegrees / SwerveConstants.kTurningEncoderDegreesPerTick);
    DataLogUtil.writeMessageEcho(
        buildString("SwerveModule ", swName),
        ": calibrateTurningEncoder, turningEncoderZero =",
        turningEncoderZero,
        ", raw encoder =",
        getTurningEncoderRaw(),
        ", set degrees =",
        currentAngleDegrees,
        ", encoder degrees =",
        getTurningEncoderDegrees());
  }

  /**
   * Gets the turning motor encoder facing. When calibrated, 0 should be with the wheel pointing
   * toward the front of robot (positive = counterclockwise, negative = clockwise).
   *
  ...
  public double getTurningEncoderDegrees() {
    return (getTurningEncoderRaw() - turningEncoderZero)
        * SwerveConstants.kTurningEncoderDegreesPerTick;
  }

  /**
   * Converts a target wheel facing (in degrees) to a target raw turning motor encoder value.
   *
   * @param targetDegrees desired wheel facing, in degrees (0 = towards front of robot, positive =
   ...
   *
   * @return turning TalonFX encoder rotational velocity for wheel facing, in degrees/second
   *     (positive = counterclockwise, negative = clockwise)
   */
  public double getTurningEncoderVelocityDPS() {
    turningEncoderVelocity.refresh();
    return turningEncoderVelocity.getValueAsDouble()
        * SwerveConstants.kTurningEncoderDegreesPerTick;
  }
  ...
  /**
   * Calibrates the CANcoder encoder so that 0 should be with the wheel pointing toward the front of
   * robot.
   *
  ...
  public void calibrateCanCoderDegrees(double offsetDegrees) {
    ...
    DataLogUtil.writeMessageEcho(
        buildString("SwerveModule ", swName),
        ": calibrateCanCoder, cancoderZero =",
        cancoderZero,
        ", raw encoder =",
        turningCanCoderPosition.refresh().getValueAsDouble() * 360.0,
        ", encoder degrees =",
        getCanCoderDegrees());
  }

  /**
   * Gets the wheel facing of the turning motor. When calibrated, 0 should be with the wheel
   * pointing toward the front of robot (positive = counterclockwise, negative = clockwise).
   *
   * @return wheel facing, in degrees [-180,+180)
   */
  public double getCanCoderDegrees() {
    turningCanCoderPosition.refresh();
    return MathBCR.normalizeAngle(
        turningCanCoderPosition.getValueAsDouble() * 360.0 - cancoderZero);
  }

  /**
   * Gets the rotatioal velocity of the turning CANcoder.
   *
   * @return rotational velocity for wheel facing, in degrees/second (positive = counterclockwise,
   *     negative = clockwise)
   */
  ...
  public void updateShuffleboard() {
    SmartDashboard.putNumber(
        buildString("Swerve FXangle ", swName), MathBCR.normalizeAngle(getTurningEncoderDegrees()));
    SmartDashboard.putNumber(buildString("Swerve CCangle ", swName), getCanCoderDegrees());
    SmartDashboard.putNumber(
        buildString("Swerve FXangle dps", swName), getTurningEncoderVelocityDPS());
