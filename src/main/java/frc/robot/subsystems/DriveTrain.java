...
public class DriveTrain extends SubsystemBase implements Loggable {
  ...
  private final StatusSignal<Angle> pigeonPitch =
      pigeon.getRoll(); // Pigeon is mounted rotated by 90deg, so robot pitch is pigeon roll
  ...
  private final Matrix<N3, N1> closeMatrix =
      new Matrix<>(Nat.N3(), Nat.N1(), new double[] {1, 1, Math.PI});
  private final Matrix<N3, N1> farMatrix =
      new Matrix<>(Nat.N3(), Nat.N1(), new double[] {2, 2, 2 * Math.PI});

  private final DataLog log = DataLogManager.getLog();
  private final StructLogEntry<Pose2d> dLogOdometryPose2D =
      StructLogEntry.create(log, "/DrivePathFollower/curPose2d", Pose2d.struct);
  ...
  private final DoubleLogEntry dLogGyroVelocity =
      new DoubleLogEntry(log, "/DriveTrain/GyroVelocity");
  ...
  private final DoubleLogEntry dLogOdometryTheta =
      new DoubleLogEntry(log, "/DriveTrain/OdometryTheta");
  private final DoubleLogEntry dLogDriveSpeed = new DoubleLogEntry(log, "/DriveTrain/DriveSpeed");
  private final DoubleLogEntry dLogDriveXVelocity =
      new DoubleLogEntry(log, "/DriveTrain/DriveXVelocity");
  private final DoubleLogEntry dLogDriveYVelocity =
      new DoubleLogEntry(log, "/DriveTrain/DriveYVelocity");
  ...
  // private double currAng;                                            // Current recorded gyro
  // angle
  // private double prevTime;                                           // Last time gyro angle was
  // recorded
  // private double currTime;                                           // Current time gyro angle
  // is being recorded
  // private double angularVelocity;                                    // Robot angular velocity in
  // degrees per second
  // private LinearFilter lfRunningAvg = LinearFilter.movingAverage(4); // Calculate running average
  // to smooth quantization error in angular velocity calc
  ...
  private boolean useVisionForOdometry =
      false; // false = will not use vision in odometry, true = uses vision for odometry
  ...
  // private boolean elevatorUpPriorIteration = false;  // Slew rate limiter, tracking for elevator
  // position from prior iteration

  // Cache a copy of the DriveStop command for using to cancel the default command (typically
  // DriveWithJoystickAdvanced)
  ...
  public DriveTrain(AllianceSelection allianceSelection) {
    ...
    cameraLeft =
        new PhotonCameraWrapper(
            PhotonVisionConstants.robotToCamLeft,
            PhotonVisionConstants.leftAprilTagCameraName,
            logRotationKey);
    cameraRight =
        new PhotonCameraWrapper(
            PhotonVisionConstants.robotToCamRight,
            PhotonVisionConstants.rightAprilTagCameraName,
            logRotationKey);
    ...
    swerveFrontLeft =
        new SwerveModule(
            ...
            true,
            false,
            offsetAngleFrontLeftMotor,
            SwerveConstants.kVmFL);
    swerveFrontRight =
        new SwerveModule(
            "FR",
            ...
            true,
            false,
            offsetAngleFrontRightMotor,
            SwerveConstants.kVmFR);
    swerveBackLeft =
        new SwerveModule(
            "BL",
            ...
            true,
            false,
            offsetAngleBackLeftMotor,
            SwerveConstants.kVmBL);
    swerveBackRight =
        new SwerveModule(
            "BR",
            ...
            true,
            false,
            offsetAngleBackRightMotor,
            SwerveConstants.kVmBR);
    ...
    // Create and initialize odometery. Set initial location to (0,0) and facing away from driver
   ...
   *
   * @return raw gyro pitch, in degrees, from 180 to -180 (postitive = nose up, negative = nose
   *     down)
   */
  ...
  /**
   * Returns the current calibrated gyro reading. This is based only upon the gyro angle, and does
   * not take into account the robot rotation on the field as informed by the AprilTags.
   *
  ...
  public double getGyroRotation() {
    ...
    angle =
        MathBCR.normalizeAngle(
            angle); // Angle is in raw gyro units (-inf,inf), so it need to be converted to (-180,
    // 180]
    return angle;
  }
   ...
   *
   * @return gyro angular velocity, in degrees per second (positive = turning left, negative =
   *     turning right)
   */
   ...
   *
   * @return angular velocity, in degrees per second (positive = turning left, negative = turning
   *     right)
   */
   ...
   * automatically configured in the SwerveModule constructors. So, this method should not need to
   * be called.
   *
   ...
   *
   * <p><b>NORE:</b> This procedure includes multiple blocking calls and will delay robot code by
   * ~250ms if the mode is changed. However, if setCoast is the same as the current setting, then
  ...
  public boolean isDriveModeCoast() {
    return swerveFrontLeft.isMotorModeCoast()
        && swerveFrontRight.isMotorModeCoast()
        && swerveBackLeft.isMotorModeCoast()
        && swerveBackLeft.isMotorModeCoast();
  }
   ...
   *
   * <p><b>NOTE:</b> This sets the absolute facing. It does <b>note</b> turn the wheels to the
   * "optimized" facing +/-180 degrees if that is closer.
   *
   * @param angle desired wheel facing relative to front of chassis, in degrees, -180 to +180
   *     (positive = left, negative = right, 0 = facing front of robot)
   */
   ...
   *
   * @param desiredStates desired swerve module states (0 = FrontLeft, 1 = FrontRight, 2 = BackLeft,
   *     3 = BackRight)
  ...
  public void setModuleStates(SwerveModuleState[] desiredStates, boolean isOpenLoop) {
    // Convert states to chassisspeeds and slew limit velocities (limit acceleration) to avoid
    // tipping the robot
   ...
   *
   * @return module states, as measured by the encoders (0 = FrontLeft, 1 = FrontRight, 2 =
   ...
   *
   * @return module positions, as measured by the encoders (0 = FrontLeft, 1 = FrontRight, 2 =
   *     BackLeft, 3 = BackRight)
   */
   ...
   *
   * @param xSpeed speed of the robot in the x direction, in meters/second (positive = move towards
   *     red driver station)
   * @param ySpeed speed of the robot in the y direction, in meters/second (positive = move left in
   *     reference to the blue driver station)
   * @param rot angular rate of the robot, in radians/second (positive = turn to the left)
   * @param fieldRelative true = the provided x and y speeds are relative to the field, false = the
   *     provided x and y speeds are relative to the current facing of the robot
   * @param isOpenLoop true = fixed drive percent output to approximate velocity, false = closed
   *     loop drive velocity control
   */
  public void drive(
   ...
   *     <p>For example, if you set the center of rotation at one corner of the robot and XSpeed,
   *     YSpeed = 0, then the robot will rotate around that corner. This feature may not work if
   *     fieldRelative = true (need to test)
   * @param fieldRelative true = the provided x and y speeds are relative to the field, false = the
   *     provided x and y speeds are relative to the current facing of the robot
   * @param isOpenLoop true = fixed drive percent output to approximate velocity, false = closed
   *     loop drive velocity control
    ...
    chassisSpeed =
        fieldRelative
            ? ChassisSpeeds.fromFieldRelativeSpeeds(
                xSpeed, ySpeed, rot, Rotation2d.fromDegrees(getGyroRotation()))
            : new ChassisSpeeds(xSpeed, ySpeed, rot);

    SwerveModuleState[] swerveModuleStates =
        kDriveKinematics.toSwerveModuleStates(chassisSpeed, centerOfRotationMeters);
   ...
   *     <p>Robot X location in the field, in meters (0 = field edge in front of the Blue driver
   *     station, positive = away from the Blue drivestation)
   *     <p>Robot Y location in the field, in meters (0 = right edge of field when standing in the
   *     Blue driver station, positive = left when looking from the Blue drivestation)
   *     <p>Robot angle on the field (0 = facing away from the Blue drivestation, positive = to the
   ...
   *
   * @return robot angle on the field, in degrees (0 = facing away from the Blue drivestation,
  ...
  /**
   * Resets the pose estimator to the specified pose (defines the robot's position and orientation
   * on the field). This method also resets the gyro, which is required for the pose to properly
   * reset.
   *
  ...
  public void resetPose(Pose2d pose) {
    DataLogUtil.writeMessage(
        "Drive.resetPose with set angle =",
    ...
    poseEstimator.resetPosition(
        Rotation2d.fromDegrees(getGyroRotation()), getModulePositions(), pose);
  }

  /**
   * Resetter used in alliance selection to properly reset the angle when the alliance of the robot
   * changes.
   *
  ...
  public void resetPoseForAlliance(Alliance alliance) {
    ...
    // TODO check if this works correctly! It should work if the DriverStation is set to Red and
    // Shuffleboard = Auto Alliance when the robot boots
    // And it should still work if the alliance changes on Shuffleboard.

    // When the alliance changes (Blue -> Red or Red -> Blue), rotate the robot orientation by 180
    // degrees.
    // However, only reset pose if robot is not in the valid field area (such as when robot code
    // boots).
    // Otherwise, the robot is on the field, so don't change anything regardless of the alliance
    // change.
    if (pose.getY() < 0.2 && pose.getX() < 0.2) {
      ...
      // DriveWithJoystickAdvanced will try to turn the robot 180 degrees to maintain the robot
      // heading.
   ...
   *
   * <p>Speed of the robot in the x direction, in meters per second (+ = away from the Blue
   * drivestation)
   *
   * <p>Speed of the robot in the y direction, in meters per second (+ = left when looking from the
   * Blue drivestation)
   *
  ...
  public ChassisSpeeds getRobotSpeeds() {
    return ChassisSpeeds.fromRobotRelativeSpeeds(
        getChassisSpeeds(), Rotation2d.fromDegrees(getGyroRotation()));
  }
  ...
  /**
   * Checks if the CANbus and gyro are working. Sometimes, when the robot boots up, either the
   * CANbus or the gyro don't initialize properly. ROBOT CODE WILL NOT BE ABLE TO CONTROL MOTORS
   * when this happens, so always check this before starting a match!
   *
   * @return true = something is not working, false = CANbus and gyro are both working
   */
  public boolean canBusError() {
    return ((swerveFrontLeft.getDriveBusVoltage() < 7.0)
        || (swerveFrontLeft.getDriveTemp() < 5.0)
        || !isGyroReading());
  }
  ...
  public void periodic() {
    ...
    if (fastLogging || DataLogUtil.isMyLogRotation(logRotationKey)) {
      ...
      // SmartDashboard.putNumber("Drive Average Dist in Meters",
      // Units.inchesToMeters(getAverageDistance()));
      ...
      // swerveFrontLeft.getTurningEncoderDegrees()*(Math.PI/180), null);
      //     builder.addDoubleProperty("Front Left Velocity", () ->
      // swerveFrontLeft.getDriveEncoderVelocity(), null);

      //     builder.addDoubleProperty("Front Right Angle", () ->
      // swerveFrontRight.getTurningEncoderDegrees()*(Math.PI/180), null);
      //     builder.addDoubleProperty("Front Right Velocity", () ->
      // swerveFrontRight.getDriveEncoderVelocity(), null);

      //     builder.addDoubleProperty("Back Left Angle", () ->
      // swerveBackLeft.getTurningEncoderDegrees()*(Math.PI/180), null);
      //     builder.addDoubleProperty("Back Left Velocity", () ->
      // swerveBackLeft.getDriveEncoderVelocity(), null);

      //     builder.addDoubleProperty("Back Right Angle", () ->
      // swerveBackRight.getTurningEncoderDegrees()*(Math.PI/180), null);
      //     builder.addDoubleProperty("Back Right Velocity", () ->
      // swerveBackRight.getDriveEncoderVelocity(), null);
  ...
  /**
   * Gets a string with the absolute value of the output percentages of all 4 swerve drive motors,
   ...
   * Increases the standard deviation when the robot is farther from the AprilTag (and hence the
   * camera is less accurate).
   *
      ...
      if (camera.hasInit()) {
        ...
        if (camResult != null) {
          Optional<EstimatedRobotPose> result =
              camera.getEstimatedGlobalPose(poseEstimator.getEstimatedPosition(), camResult);
          if (result.isPresent()) {
            EstimatedRobotPose camPose = result.get();
            SmartDashboard.putNumber(
                "Vision " + camera.getCameraName() + " X", camPose.estimatedPose.toPose2d().getX());
            SmartDashboard.putNumber(
                "Vision " + camera.getCameraName() + " Y", camPose.estimatedPose.toPose2d().getY());
            SmartDashboard.putNumber(
                "Vision " + camera.getCameraName() + " rot",
            ...
            if (camResult.hasTargets() && useVisionForOdometry) {
              ...
              if (bestTarget.getBestCameraToTarget().getX() < 1.5) {
                poseEstimator.addVisionMeasurement(
                    camPose.estimatedPose.toPose2d(), camPose.timestampSeconds, closeMatrix);
              } else if (bestTarget.getBestCameraToTarget().getX() < 3) {
                poseEstimator.addVisionMeasurement(
                    camPose.estimatedPose.toPose2d(), camPose.timestampSeconds, farMatrix);
              }
              SmartDashboard.putNumber(
                  "Vision " + camera.getCameraName() + " AprilTag", bestTarget.getFiducialId());
            } else {
