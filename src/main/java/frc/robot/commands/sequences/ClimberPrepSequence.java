...
public class ClimberPrepSequence extends SequentialCommandGroup {
  /**
   * Prepares to latch on to the cage to climb by moving the wrist and elevator to the correct
   * position, and then moving the climber to the correct position.
   *
  ...
  public ClimberPrepSequence(Elevator elevator, Wrist wrist, Climber climber) {
    addCommands(
        ...
        sequence(
                parallel(
                    runOnce(
                        () -> LEDEventUtil.sendEvent(LEDEventUtil.StripEvents.CLIMBER_PREPPING)),
                    // new WristElevatorSafeMove(ElevatorWristPosition.START_CONFIG,
                    // RegionType.CORAL_ONLY, elevator, wrist),   // Do not move elevator/wrist, in
                    // case co-driver preps while scoring.  Co-driver is responsible for
                    // elevator/wrist.
