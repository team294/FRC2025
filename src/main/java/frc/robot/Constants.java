...
public final class Constants {
  ...
  public enum CoordType {
    ...
    kAbsoluteResetPoseTol; // Absolute field coordinates, reset robot pose if robot is not close to
    // specified position
  }
  ...
  public static final class Ports {
    ...
    // Note: Remote sensors accessed by a Talon FX (Falcon 500) must have a CAN ID of 15 or less.
    // See errata
  ...
  public static final class RobotDimensions {
    ...
    public static final double DRIVETRAIN_TRACKWIDTH_METERS =
        Units.inchesToMeters(23.75) * DrivetrainAdjustmentFactor;
    ...
    public static final double robotWidth =
        Units.inchesToMeters(35.5); // CALIBRATED (for DriveToReef)
  ...
  public static final class SwerveConstants {
    // Past kDriveGearRatio: L3 = 6.12:1. Mk4i = 8.14:1 (L1-std gears), 6.75:1 (L2-fast gears),
    // 5.903 (modified L2 16-tooth gear).
    // Past kTurningGearRatio: Mk4i = 150 / 7:1.

    // Encoder calibration to meters traveled or wheel facing degrees
    public static final double kEncoderCPR =
        1.0; // CALIBRATED Encoder counts per revolution of motor pinion gear
    ...
    public static final double kWheelDiameterMeters =
        Units.inchesToMeters(4)
            * 0.9554; // CALIBRATED Wheels are nominal 4".  LAR was 0.9683, AVR is 0.9554
    public static final double kDriveEncoderMetersPerTick =
        (kWheelDiameterMeters * Math.PI) / kEncoderCPR / kDriveGearRatio;
    public static final double kTurningEncoderDegreesPerTick =
        360.0 / kEncoderCPR / kTurningGearRatio;
    ...
    // and ensures that the robot travels in the requested direction.  So, use min value of all 4
    // motors,
    ...
    public static final double kFullAccelerationMetersPerSecondSquare =
        0.9 * kMaxAccelerationMetersPerSecondSquare;
    ...
    public static final double kMaxAngularAccelerationRadiansPerSecondSquared =
        35.0; // Not used in code currently
    public static final double kNominalAngularAccelerationRadiansPerSecondSquared = Math.PI;

    // CALIBRATED
    public static final double kVDriveAvg =
        2.344; // In voltage per meters/second    LAR was 2.251 (FOC off), FOC cal is 2.392 (FOC
    // on), turn for AVR is 2.344 (FOC on)
    ...
    public static final double dt =
        0.02; // CALIBRATED Timestep for discretizing robot motion, in seconds (scheduler time
    // period = 20ms)
    public static final double kADrive =
        0.2383; // CALIBRATED In voltage per meters per second^2   LAR was 0.2692 (FOC off), AVR is
    // 0.2383 (FOC on)
    public static final double kADriveToPose =
        0.050; // CALIBRATED on ETU in 3/2/2025 In seconds (On the ETU, DriveToPose behaves better
    // with a small value for kADriveToPose)
    public static final double kSDrive =
        0.141; // CALIBRATED In voltage      LAR was 0.030 (FOC off), AVR is 0.141 (FOC on)

    // Minimum abs delta (in m/sec) between actual wheel velocity and desired wheel velocity for
    // kADrive to be applied.
    // If the delta is less than this, then don't use kADrive. This prevents the drive motors from
    // jittering.
    // Note that the swerve module kP in the velocity controller will still work to maintain the
    // proper wheel speed.
  ...
  public static final class DriveConstants {
    ...
    public static final SwerveDriveKinematics kDriveKinematics =
        new SwerveDriveKinematics(
            new Translation2d(
                RobotDimensions.DRIVETRAIN_WHEELBASE_METERS / 2,
                RobotDimensions.DRIVETRAIN_TRACKWIDTH_METERS / 2),
            new Translation2d(
                RobotDimensions.DRIVETRAIN_WHEELBASE_METERS / 2,
                -RobotDimensions.DRIVETRAIN_TRACKWIDTH_METERS / 2),
            new Translation2d(
    ...
    // After updating in RobotPreferences, you will need to re-start the robot code for the changes
    // to take effect.
    // When calibrating offset, set the wheels to zero degrees with the bevel gear facing to the
    // right
    ...
    public static final double distanceFromReefToScore =
        Units.inchesToMeters(
            5.0); // was 0.25 meters, changed to be scoring location for L4. 7.25 at AVR, 6.25 at
    // LAR, changed to be 5.0in on 9/16
    public static final double distanceFromReefToScoreL4 =
        Units.inchesToMeters(
            7.25); // was 0.25 meters, changed to be scoring location for L4. 7.25 at AVR, 6.25 at
    // LAR

    // How far away from the scoring prep position we start to move the elevator to the set position
    public static final double distanceFromReefToElevate =
        0.05; // Value very close to zero so it doesn't evelevate until the robot is stopped

    // Distance bumpers should be away from the reef
    // This distance is where the algae intake prep will take place for both levels
    public static final double distanceFromReefToPickupAlgaeUpper =
        Units.inchesToMeters(8.25); // Discussed 7.25 instead...
    public static final double distanceFromReefToPickupAlgaeLower =
        Units.inchesToMeters(10.75); // Discussed 7.25 instead...
  ...
  public static class FieldConstants {
    ...
    // Calculated by measuring y distance between center of reef wall and reef pole (6.469731 in),
    // converted to meters
    public static final double ReefScoringPositionAprilTagOffset = 0.164331496063;
    // Calculated by adding tag -> branch to branch -> edge, then subtracting 2 inches from testing
    // in lab (calculated number is above constant + 12.05in)
    public static final double ReefScoringPositionAprilTagOffsetL1 =
        ReefScoringPositionAprilTagOffset + Units.inchesToMeters(10.05);

    public static final double bargeScorableWidth =
        3.3; // Width of alliance barge structure (not net) is 3.72 meters, reduced for consistency
    // CALIBRATED 4/12/2025
  ...
  public static class VisionConstants {
    public static class PhotonVisionConstants {
      public static final Transform3d robotToCamLeft =
          new Transform3d(
              new Translation3d(
                  Units.inchesToMeters(9.72 - 0.8 + 0.6),
                  Units.inchesToMeters(12.02 + 0.4),
                  Units.inchesToMeters(12.7204)), // had -0.5 to Y at AVR
              new Rotation3d(
                  0,
                  Units.degreesToRadians(0),
                  Units.degreesToRadians(
                      -34.5))); // Cam mounted facing forward-right on the left side of the robot
      public static final Transform3d robotToCamRight =
          new Transform3d(
              ...
              new Rotation3d(
                  ...
                  Units.degreesToRadians(0),
                  Units.degreesToRadians(
                      30.5 + 1.0))); // Cam mounted facing forward-left on the right side of the
      // robot
  ...
  public static final class CoralEffectorConstants {
    ...
    public static final double fastIntakePercent =
        0.22; // CALIBRATED for LAR (however, due to max voltage on motor, was really capped at
    // 0.167).  4/18 reduced from 0.25 to 0.22 to reduce "coral pass-through" on coral
    // loading.
    ...
    public static final double centerRotationsUndershoot =
        1.625; // CALIBRATED #3  Measure the typical undershoot with kP.  This value (in motor
    // rotataions) will be added to the encoder reading when the coral sensor is
    // triggered.
    public static final double centeringTolerance =
        0.07; // CALIBRATED #1  Position tolerance (in rotations) for holding coral [smaller than
    // 1/2 of the position window where both sensors see the coral]
    public static final double centeringStepSize =
        centeringTolerance
            * 0.5; // #4 If the autoHold oscillates (not setPosition oscillating), then reduce this
    // step size.

    public static final double kP =
        5.0; // CALIBRATE #2 (in Phoenix tuner)   kP = (desired-output-volts) /
    // (error-in-encoder-rotations)
    public static final double kI = 0.0; // CALIBRATED
    public static final double kD = 0.0; // CALIBRATED
    public static final double kG =
        0.0; // CALIBRATED #5 if needed.  (start with +0.009 * 12 = +0.108 ?)  Or measure holding
    // voltage to keep coral when vertical (and kP turned off)
    //  #6 For kG, add an offset angle to true vertical coral, if needed.  (in coral.setposition
  ...
  public static final class AlgaeGrabberConstants {
    ...
    public static final double netOuttakePercent =
        -0.27; // CALIBRATED 4/12.  4/18:  Was -0.35, now -0.20.  C2 increased to 0.23, 9/27/25
    // increased to 0.27 (chezy champs)
    public static final double processorOuttakePercent =
        -0.18; // CALIBRATED 3/29.  Updated 4/16 from -0.15 to -0.18
  }

  public static final class ElevatorConstants {
    public static final double kEncoderCPR =
        1.0; // Encoder counts per revolution of the motor pinion gear
    public static final double kElevGearRatio =
        (5.0 / 1.0); // Gear reduction ratio between Kraken and gear driving the elevator CALIBRATED
    // FOR 2025 (5:1)
    public static final double kElevPulleyDiameterInches =
        1.504; // Diameter of the pulley driving the elevator in inches (1.504" nominal)  GOAL
    // HIEGHTS CALIBRATED USING THIS VALUE
    public static final double kElevEncoderInchesPerTick =
        (kElevPulleyDiameterInches * Math.PI) / kEncoderCPR / kElevGearRatio;

    public static final double compensationVoltage = 12.0;

    public static final double maxUncalibratedPercentOutput = 0.05;
    public static final double maxManualPercentOutput =
        0.1; // Max elevator speed when driven using Xbox controller
  ...
  public static final class WristConstants {
    // Gear Ratio (convention from CTRE library) = the ratio of motor rotor rotations to wrist
    // rotations,
    // where a ratio greater than 1 is a reduction.
    public static final double kWristGearRatio =
        ((60.0 / 8.0) * (48.0 / 16.0)); // CALIBRATED FOR 2025 (22.5:1)
    public static final double kWristDegreesPerRotation =
        360.0; // Wrist degrees per rotation of the cancoder
    ...
    // When wrist coral top metal plate is horizontal (bubble level), wrist angle should read 100.6
    // degrees
    public static double offsetAngleCANcoder =
        -31.5; // CALIBRATED.   CANCoder raw angle (in degrees) when arm is at 0 degrees.

    // 1 makes absolute position unsigned [0, 1); 0.5 makes it signed [-0.5, 0.5), 0 makes it always
    // negative
    public static double cancoderDiscontinuityPoint =
        0.50; // CALIBRATED - should be the center of the region of unallowed motion

    public static final double kP =
        40.0; // CALIBRATED    kP = (desired-output-volts) / (error-in-wrist-rotations)
    public static final double kI = 0.0; // CALIBRATED
    public static final double kD = 0.0; // CALIBRATED
    public static final double kG =
        0.522; // CALIBRATED    kG = Feed foward voltage to add to hold wrist horizontal (0 deg)
    ...
    public static final double MMCruiseVelocity =
        0.80; // Max velocity in wrist rotations / second CALIBRATED  TODO opportunity to make this
    // faster
    public static final double MMAcceleration =
        MMCruiseVelocity
            / 0.35; // Max acceleration in wrist rotations / second^2. MMVel / MMAccel = seconds to
    // full velocity. CALIBRATED
    ...
    public enum WristAngle {
      LOWER_LIMIT(
          -13.0), // Wrist chin strap limit (without coral = -26.5 deg).  Elevator needs to be above
      // 5.4" to get to this angle.
      UPPER_LIMIT(100.5);

      @SuppressWarnings({"MemberName", "PMD.SingularField"})
      public final double value;

      WristAngle(double value) {
        this.value = value;
      }
    }
  }
  ...
  public static final class ElevatorWristConstants {
    public enum ElevatorWristPosition {
      ...
      CORAL_L2(
          22.56,
          95.0), // CALIBRATED ON 4/7. Was 65.0 degrees, adjusted to be 1 coral away from reef.
      // 4/16 elevator increased from 22.06 to 22.56. 4/18 elevator increased from 22.56
      // to 23.31 to 23.56.  9/16 elevator decreased from 23.56 to 22.56 to account for
      // decrease in distance from reef while scoring.
      CORAL_L3(
          38.03,
          95.0), // CALIBRATED ON 4/7. Was 65.0 degrees, adjusted to be 1 coral away from reef.
      // 4/16 elevator increased from 37.28 to 37.78. 4/18 elevator increased from 37.78
      // to 38.03 to 38.53.  9/16 elevator decreased from 38.53 to 38.03 to account for
      // decrease in distance from reef while scoring.
      CORAL_L4(71.0, 57.0),
      ...
      ALGAE_LOWER(
          22.05,
          24.0), // elevator position was 22.3 inches, increased to 22.55 to prevent wheel rubbing
      // against reef pole, 9/28/25 Chezy - decreased to 22.05 (from 22.55) as it looked
      // a little high
  ...
  public static final class ClimberConstants {
    ...
    public static final double kClimberDegreesPerRotation =
        360.0; // Wrist degrees per rotation of the CANcoder

    public static final double compensationVoltage = 12.0;
    public static final double maxUncalibratedPercentOutput = 0.1; // CALIBRATED
    public static final double maxManualPercentOutput =
        0.22; // CALIBRATED.  4/17 increased from 0.22 to 0.30.  4/18:  moved back to 0.22
    public static final double maxPercentOutput =
        0.40; // Tested in lab 10/28, increased from 0.22 to 0.40      4/16 increased from 0.20 to
    // 0.22.  4/17 increased from 0.22 to 0.30.  4/18: moved back to 0.22

    // Should be updated in RobotPreferences, so it cannot be final
    public static double offsetAngleCANcoder =
        -232.734; // CANCoder raw angle (in degrees) when arm is at 0 degrees. CALIBRATED 4/12

    // 1 makes absolute position unsigned [0, 1); 0.5 makes it signed [-0.5, 0.5), 0 makes it always
    // negative
    // This value is the center of the region of *unallowed* motion
    public static double cancoderDiscontinuityPoint =
        0.071; // CALIBRATED 4/13 - minimum raw cc is 0.308, maximum raw cc is 0.834

    public static final double kP =
        (compensationVoltage * maxPercentOutput)
            / 0.01; // CALIBRATED 4/11. kP = (desired-output-volts) / (error-in-wrist-rotations)
    public static final double kI = 0.0; // CALIBRATED
    public static final double kD = 0.0; // CALIBRATED
    public static final double kG =
        0.0; // CALIBRATED   kG = Feed foward voltage to add to hold wrist horizontal (0 deg)
    public static final double kS = 0.096; // CALIBRATED   kS = (volts)
    public static final double kV =
        15.2 * 192.0 / 135.0; // CALIBRATED   kV = (volts)/(wrist-rotations/sec)

    public static final double MMCruiseVelocity =
        50.0 / 360.0; // Max velocity in climber rotations / second
    public static final double MMAcceleration =
        MMCruiseVelocity
            / 0.35; // Max acceleration in climber rotations / second^2. MMVel / MMAccel = seconds
    // to full velocity.
    public static final double MMJerk =
        MMAcceleration
