...
public class Climber extends SubsystemBase implements Loggable {
  ...
  // We create two configs for the climber motor -- one with RemoteCANcoder and one with
  // RotorEncoder.
  // The default config is RemoteCANcoder. If the CANcoder fails, the climber will the
  // use the RotorEncoder config as a fallback.
  private boolean usingCANcoder; // true = using CANcoder configuration, false = using RotorEncoder
  // configuration
  private TalonFXConfiguration
      climberMotor_RotorEncoderConfig; // Configuration reading the rotor encoder
  ...
  private MotionMagicVoltage climberMMVoltageControl =
      new MotionMagicVoltage(0.0).withEnableFOC(true);

  // Create variables for the climber Kraken motor
  private final StatusSignal<Voltage>
      climberSupplyVoltage; // Incoming bus voltage to motor, in volts
  ...
  private final StatusSignal<Current>
      climberStatorCurrent; // Motor stator current, in amps (positive = forward, negative =
  // reverse)
  ...
  private double encoderZero =
      0; // Reference raw encoder angle for climber encoder (may be motor built-in encoder or remote
  // CANcoder) at 0 degrees.

  private boolean climberCalibrated =
      false; // Default to climber being uncalibrated. Calibrate from robot preferences or
  // "Calibrate Climber Zero" button on dashboard.

  private double
      safeAngle; // Current climber target on position control on the motor (if in position mode)
  ...
  private final DoubleLogEntry dLogTemp = new DoubleLogEntry(log, "/Climber/Temp");
  private final DoubleLogEntry dLogPctOut = new DoubleLogEntry(log, "/Climber/PctOut");
  private final DoubleLogEntry dLogCurrent = new DoubleLogEntry(log, "/Climber/Current");
  // private final DoubleLogEntry dLogVolt = new DoubleLogEntry(log, "/Climber/VoltOut");
  private final DoubleLogEntry dLogBusVolt = new DoubleLogEntry(log, "/Climber/BusVolt");
  private final DoubleLogEntry dLogCancoderRaw = new DoubleLogEntry(log, "/Climber/CancoderRaw");
  // private final DoubleLogEntry dLogEncRaw = new DoubleLogEntry(log, "/Climber/EncRaw");
  private final DoubleLogEntry dLogCalZero = new DoubleLogEntry(log, "/Climber/CalZero");
  private final DoubleLogEntry dLogTargetAngle = new DoubleLogEntry(log, "/Climber/TargetAngle");
  // private final DoubleLogEntry dLogEncAngle = new DoubleLogEntry(log, "/Climber/EngAngle");
  private final DoubleLogEntry dLogCurAngle = new DoubleLogEntry(log, "/Climber/CurrentAngle");
  private final DoubleLogEntry dLogVel = new DoubleLogEntry(log, "/Climber/VelocityDPS");
  private final DoubleLogEntry dLogAccel = new DoubleLogEntry(log, "/Climber/AccelDPS2");
  private final BooleanLogEntry dLogCancoderConnected =
      new BooleanLogEntry(log, "/Climber/CancoderConnected");
  ...
  public Climber(String subsystemName) {
    ...
    climberMotor_RotorEncoderConfig.Voltage.PeakForwardVoltage =
        ClimberConstants.compensationVoltage;
    climberMotor_RotorEncoderConfig.Voltage.PeakReverseVoltage =
        -ClimberConstants.compensationVoltage;
    climberMotor_RotorEncoderConfig.OpenLoopRamps.VoltageOpenLoopRampPeriod =
        0.3; // Time from 0 to full power, in seconds
    climberMotor_RotorEncoderConfig.ClosedLoopRamps.VoltageClosedLoopRampPeriod =
        0.3; // Time from 0 to full power, in seconds
    ...
    climberMotor_RotorEncoderConfig.CurrentLimits.SupplyCurrentLimit =
        60.0; // Upper limit for the current, in amps
    climberMotor_RotorEncoderConfig.CurrentLimits.SupplyCurrentLowerLimit =
        35.0; // Lower limit for the current, in amps
    climberMotor_RotorEncoderConfig.CurrentLimits.SupplyCurrentLowerTime =
        0.2; // Threshold time, in seconds
    climberMotor_RotorEncoderConfig.CurrentLimits.SupplyCurrentLimitEnable = true;

    climberMotor_RotorEncoderConfig.Slot0.kP =
        ClimberConstants.kP; // kP = (desired-output-volts) / (error-in-encoder-rotations)
    ...
    climberMotor_RotorEncoderConfig.MotionMagic.MotionMagicCruiseVelocity =
        ClimberConstants.MMCruiseVelocity;
    climberMotor_RotorEncoderConfig.MotionMagic.MotionMagicAcceleration =
        ClimberConstants.MMAcceleration;
    ...
    climberMotor_RotorEncoderConfig.Feedback.FeedbackSensorSource =
        FeedbackSensorSourceValue.RotorSensor;
    climberMotor_RotorEncoderConfig.Feedback.RotorToSensorRatio = 1.0;
    climberMotor_RotorEncoderConfig.Feedback.SensorToMechanismRatio =
    ...
    climberMotor_CANcoderConfig.OpenLoopRamps.VoltageOpenLoopRampPeriod =
        0.3; // Time from 0 to full power, in seconds
    climberMotor_CANcoderConfig.ClosedLoopRamps.VoltageClosedLoopRampPeriod =
        0.3; // Time from 0 to full power, in seconds
    ...
    // If the current is above the supply current limit for the threshold time, the current is
    // limited to the lower limit in order to prevent the breakers from tripping
    climberMotor_CANcoderConfig.CurrentLimits.SupplyCurrentLimit =
        60.0; // Upper limit for the current, in amps
    climberMotor_CANcoderConfig.CurrentLimits.SupplyCurrentLowerLimit =
        35.0; // Lower limit for the current, in amps
    climberMotor_CANcoderConfig.CurrentLimits.SupplyCurrentLowerTime =
        0.2; // Threshold time, in seconds
    climberMotor_CANcoderConfig.CurrentLimits.SupplyCurrentLimitEnable = true;

    climberMotor_CANcoderConfig.Slot0.kP =
        ClimberConstants.kP; // kP = (desired-output-volts) / (error-in-encoder-rotations)
    ...
    // Configure Magic Motion settings
    climberMotor_CANcoderConfig.MotionMagic.MotionMagicCruiseVelocity =
        ClimberConstants.MMCruiseVelocity;
    climberMotor_CANcoderConfig.MotionMagic.MotionMagicAcceleration =
        ClimberConstants.MMAcceleration;
    climberMotor_CANcoderConfig.MotionMagic.MotionMagicJerk = ClimberConstants.MMJerk;

    // Configure encoder to user for feedback
    climberMotor_CANcoderConfig.Feedback.FeedbackSensorSource =
    ...
    canCoderConfig.MagnetSensor.AbsoluteSensorDiscontinuityPoint =
        ClimberConstants.cancoderDiscontinuityPoint;
    ...
    // may contain an old value, not the value based on the updated configuration settings above!!!!
    //  The CANBus runs
    // asynchronously from this code, so sending the updated configuration to the CANcoder/TalonFX
    // and then receiving
    // an updated position measurement back may take longer than this code. The timeouts in the
    // configuration code
    ...
    if (usingCANcoder) {
      ...
      DataLogUtil.writeMessageEcho(
          "Climber: CANcoder calibrated, Enc Zero =",
          encoderZero,
          ", CANcoder Rot =",
          getCANcoderRotationsRaw(),
          ", Enc Raw =",
          getClimberEncoderRotationsRaw(),
          ", Angle =",
          getClimberAngle(),
          ", Target =",
          getCurrentClimberTarget());

      // Set software limits after setting encoderZero
      climberMotor_CANcoderConfig.SoftwareLimitSwitch.ForwardSoftLimitThreshold =
          climberDegreesToEncoderRotations(ClimberAngle.UPPER_LIMIT.value);
      climberMotor_CANcoderConfig.SoftwareLimitSwitch.ReverseSoftLimitThreshold =
          climberDegreesToEncoderRotations(ClimberAngle.LOWER_LIMIT.value);
      ...
      // NOTE!!! When the TalonFX encoder settings are changed above, then the next call to
      // getTurningEncoderDegrees()
      // may contain an old value, not the value based on the updated configuration settings
      // above!!!!  The CANBus runs
      // asynchronously from this code, so sending the updated configuration to the CANcoder/TalonFX
      // and then receiving
      // an updated position measurement back may take longer than this code. The timeouts in the
      // configuration code
      ...
      DataLogUtil.writeMessageEcho("Climber: CANcoder not calibrated");
  ...
  public void setClimberPercentOutput(double percent) {
    if (climberCalibrated) {
      percent =
          MathUtil.clamp(
              percent, -ClimberConstants.maxPercentOutput, ClimberConstants.maxPercentOutput);
    } else {
      percent =
          MathUtil.clamp(
              percent,
              -ClimberConstants.maxUncalibratedPercentOutput,
              ClimberConstants.maxUncalibratedPercentOutput);
    }
    ...
    climberMotor.setControl(
        climberVoltageControl.withOutput(percent * ClimberConstants.compensationVoltage));
  }
  ...
  /**
   * Sets the climber angle, only when the encoder is working and calibrated, operating with
   * interlocks.
   *
  ...
  /**
   * Sets the angle of the climber only when the encoder is working and calibrated, operating with
   * interlocks.
   *
  ...
  public void setClimberAngle(double angle) {
    if (climberCalibrated && getRatchetPosition() == ServoPosition.DISENGAGED) {
      ...
      safeAngle =
          MathUtil.clamp(
              angle,
              ClimberConstants.ClimberAngle.LOWER_LIMIT.value,
              ClimberConstants.ClimberAngle.UPPER_LIMIT.value);

      // Phoenix6 PositionVoltage control: Position is in rotor rotations, FeedFoward is in Volts
      climberMotor.setControl(
          climberMMVoltageControl
              .withPosition(climberDegreesToEncoderRotations(safeAngle))
              .withFeedForward(ClimberConstants.kG * Math.cos(safeAngle * Math.PI / 180.0)));

      DataLogUtil.writeMessage(
          subsystemName, ": setClimberAngle, Desired Angle =", angle, ", Set Angle =", safeAngle);
      SmartDashboard.putNumber("Climber set raw rot", climberDegreesToEncoderRotations(safeAngle));
    }
  }

  /**
   * Gets the angle that the climber is trying to move to. If the climber is not calibrated, then
   * this value is the lowerLimit, since we really do not know where the climber is at. If the
   * climber is in manual control mode, then this value is the actual climber position.
   *
  ...
  public double getCurrentClimberTarget() {
    ...
    if (climberCalibrated) {
      if (isClimberMotorPositionControl()) {
        ...
        // If we are not in position control mode, then we are not moving towards a target (and the
        // target
      ...
      // Climber is not calibrated. Assume we are at lowerLimit to engage all interlocks, since we
      // really
  ...
  // ****** Kraken encoder methods (may be reading internal encoder or may be reading remote
  // CANcoder)
  ...
  private double getClimberEncoderDegrees() {
    // DO NOT normalize this angle. It should not wrap, since the climber mechanically can not cross
    // the -180/+180 degree point.
    return getClimberEncoderRotationsRaw() * ClimberConstants.kClimberDegreesPerRotation
        - encoderZero;
  }

  /**
   * Returns the angle that the climber is currently positioned at. If the climber is not
   * calibrated, then this value is the lowerLimit, since we really do not know where the climber is
   * at.
   *
   * @return current climber angle, in degrees (0 = horizontal in front of robot, positive = up,
   *     negative = down)
   */
  ...
  public void setClimberUncalibrated() {
    ...
    DataLogUtil.writeMessageEcho(
        "Climber: Uncalibrate, CANcoder Rot =",
        getCANcoderRotationsRaw(),
        ", Enc Raw =",
        getClimberEncoderRotationsRaw(),
        ", Angle =",
  ...
  /**
   * Calibrates the climber encoder, assuming we know the its current angle, and sets
   * climberCalibrated to true.
   *
  ...
  public void calibrateClimberEncoder(double angle) {
    ...
    encoderZero =
        getClimberEncoderRotationsRaw() * ClimberConstants.kClimberDegreesPerRotation - angle;
    climberCalibrated = true;

    DataLogUtil.writeMessageEcho(
        "Climber: Calibrate, Using CANcoder =",
        usingCANcoder,
        ", Enc Zero =",
        encoderZero,
        ", CANcoder Rot =",
    ...
    if (usingCANcoder) {
      ...
      // Set software limits after setting encoderZero
      climberMotor_RotorEncoderConfig.SoftwareLimitSwitch.ForwardSoftLimitThreshold =
          climberDegreesToEncoderRotations(ClimberAngle.UPPER_LIMIT.value);
      climberMotor_RotorEncoderConfig.SoftwareLimitSwitch.ReverseSoftLimitThreshold =
          climberDegreesToEncoderRotations(ClimberAngle.LOWER_LIMIT.value);
  ...
  public void updateClimberLog(boolean logWhenDisabled) {
    if (logWhenDisabled || !DriverStation.isDisabled()) {
      ...
      dLogTemp.append(climberTemp.refresh().getValueAsDouble(), timeNow);
      dLogPctOut.append(getClimberMotorPercentOutput(), timeNow);
      dLogCurrent.append(climberStatorCurrent.refresh().getValueAsDouble(), timeNow);
      // dLogVolt.append(climberVoltage.refresh().getValueAsDouble(), timeNow);
      dLogBusVolt.append(climberSupplyVoltage.refresh().getValueAsDouble(), timeNow);
      dLogCancoderRaw.append(getCANcoderRotationsRaw(), timeNow);
      // dLogEncRaw.append(getClimberEncoderRotationsRaw(), timeNow);
      dLogCalZero.append(encoderZero, timeNow);
      dLogTargetAngle.append(getCurrentClimberTarget(), timeNow);
      // dLogEncAngle.append(getClimberEncoderDegrees(), timeNow);
      dLogCurAngle.append(getClimberAngle(), timeNow);
      dLogVel.append(
          climberEncoderVelocity.refresh().getValueAsDouble()
              * ClimberConstants.kClimberDegreesPerRotation,
          timeNow);
      dLogAccel.append(
          climberEncoderAcceleration.refresh().getValueAsDouble()
              * ClimberConstants.kClimberDegreesPerRotation,
          timeNow);
  ...
  public void periodic() {
    if (DataLogUtil.isMyLogRotation(logRotationKey)) {
      ...
      SmartDashboard.putBoolean(
          "Climber Ratchet Disengaged",
          getRatchetPosition()
              == ServoPosition
                  .DISENGAGED); // Disengaged so red on dashboard = don't move climber down
    ...
    if (usingCANcoder && !isCANcoderConnected()) {
      ...
      DataLogUtil.writeMessageEcho(
          "Climber: CANcoder Disconnection, Enc Zero =",
          encoderZero,
          ", CANcoder Angle =",
          getCANcoderRotationsRaw(),
          ", Enc Raw =",
