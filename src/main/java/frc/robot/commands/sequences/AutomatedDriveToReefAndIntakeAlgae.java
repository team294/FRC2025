...
public class AutomatedDriveToReefAndIntakeAlgae extends SequentialCommandGroup {
  ...
  static final Map<AlgaeLocation, ElevatorWristPosition> algaeToElevatorMap =
      new EnumMap<>(AlgaeLocation.class);
   ...
   */
  public AutomatedDriveToReefAndIntakeAlgae(
      ElevatorWristPosition algaeLevel,
                ...
                sequence(
                        ...
                        parallel(
                            ...
                            sequence(
                                ...
                                new AlgaeIntakeSequence(
                                    algaeLevel, elevator, wrist, algaeGrabber))),
                        sequence(
                            new DriveToReefWithOdometryForAlgae(driveTrain, field),
                            new WristElevatorSafeMove(
                                algaeLevel, RegionType.STANDARD, elevator, wrist)),
                    ...
                    parallel(
                        new DriveToPose(
                            ...
                            () ->
                                new Pose2d(
                                    algaeLevel.equals(ElevatorWristPosition.ALGAE_LOWER)
                                        ? DriveConstants.distanceFromReefToPickupAlgaeLower
                                        : DriveConstants.distanceFromReefToPickupAlgaeUpper,
                                    0,
                                    new Rotation2d(0)),
                            0.5,
                            1.0,
                    ...
                    parallel(
                        ...
                        new WristElevatorSafeMove(
                            ...
                            wrist))),
                runOnce(
                    () -> LEDEventUtil.sendEvent(LEDEventUtil.StripEvents.AUTOMATED_DRIVING_REEF)))
            .handleInterrupt(
                () ->
                    either(
  ...
  /**
   * Drives to specific reef position and picks up algae (for auto) Does not fully stow the wrist;
   * instead, goes to ALGAE_LOWER position The robot will drive fully up against the reef, and then
   * back up after picking up the algae.
   *
   * @param algaeLevel the height of the algae, upper or lower
   * @param driveTrain DriveTrain subsystem
   * @param elevator Elevator subsystem
    ...
    addCommands(
        ...
        parallel(
            sequence(
                    ...
                    new WristElevatorSafeMove(
                        algaeToElevatorMap.get(algaeLocation),
                        RegionType.STANDARD,
                        elevator,
                        wrist),
                        ...
                        new DriveToPose(
                            ...
                            () ->
                                new Pose2d(
                                    algaeToElevatorMap
                                            .get(algaeLocation)
                                            .equals(ElevatorWristPosition.ALGAE_LOWER)
                                        ? DriveConstants.distanceFromReefToPickupAlgaeLower
                                        : DriveConstants.distanceFromReefToPickupAlgaeUpper,
                                    0,
                                    new Rotation2d(0)),
                ...
                .handleInterrupt(
                    () ->
                        either(
                            runOnce(
                                () -> LEDEventUtil.sendEvent(LEDEventUtil.StripEvents.ALGAE_MODE)),
                            runOnce(() -> LEDEventUtil.sendEvent(LEDEventUtil.StripEvents.NEUTRAL)),
                            () -> algaeGrabber.isAlgaePresent())),
            either(
                runOnce(() -> LEDEventUtil.sendEvent(LEDEventUtil.StripEvents.ALGAE_MODE)),
  ...
  /**
   * Drives to nearest reef position and picks up algae (whether algae is higher vs. lower
   * determined automatically). The robot will drive fully up against the reef, and then back up
   * after picking up the algae.
   *
   * @param driveTrain DriveTrain subsystem
   * @param elevator Elevator subsystem
   * @param wrist Wrist subsystem
                ...
                either(
                    parallel(
                        ...
                        sequence(
                            ...
                            // Move elevator/wrist to correct position based on given level
                            new AlgaeIntakeSequence(
                                field.getNearestAlgaeElevatorPosition(() -> driveTrain.getPose()),
                                elevator,
                                wrist,
                                algaeGrabber))),
                    sequence(
                    ...
                    sequence(
                        new DriveToPose(
                            ...
                            () ->
                                new Pose2d(
                                    field
                                            .getNearestAlgaeElevatorPosition(
                                                () -> driveTrain.getPose())
                                            .equals(ElevatorWristPosition.ALGAE_LOWER)
                                        ? DriveConstants.distanceFromReefToPickupAlgaeLower
                                        : DriveConstants.distanceFromReefToPickupAlgaeUpper,
