...
public class DriveResetPose extends Command {
  ...
  private final boolean
      tolerance; // true = do not reset if within 0.5m or 15 degrees of location, false = always
  // reset
   ...
   *
   * @param curXinMeters robot X location in the field, in meters (0 = field edge in front of the
   *     Blue driver station, positive = away from the Blue drivestation)
   * @param curYinMeters Robot Y location in the field, in meters (0 = right edge of field when
   *     standing in the Blue driver station, positive = left when looking from the Blue
   *     drivestation)
   * @param curAngleinDegrees robot angle on the field, in degrees (0 = facing away from the Blue
   *     drivestation, positive = to the left, negative = to the right)
   * @param tolerance true = do not reset if within 0.5m or 15 degrees of location, false = always
   *     reset
   * @param driveTrain DriveTrain subsystem
   */
  public DriveResetPose(
      double curXinMeters,
      double curYinMeters,
  ...
  public DriveResetPose(Pose2d curPose, boolean tolerance, DriveTrain driveTrain) {
    this(curPose.getX(), curPose.getY(), curPose.getRotation().getDegrees(), tolerance, driveTrain);
  }

  /**
   * Resets the pose and gyro (not the encoders) on the driveTrain. Reset the angle but keep the
   * current position (use the current measured position as the new position).
   *
   * <p><b>NOTE:</b> This command can run while the robot is disabled.
   *
   * @param curAngleinDegrees robot angle on the field, in degrees (0 = facing away from the Blue
   ...
   * <p><b>NOTE:</b> This command can run while the robot is disabled.
   *
   * @param curPose A Pose2D supplier to return the current robot pose on the field. Pose components
   *     include
   *     <p>Robot X location in the field, in meters (0 = field edge in front of the Blue driver
   *     station, positive = away from the Blue drivestation)
   *     <p>Robot Y location in the field, in meters (0 = right edge of field when standing in the
  ...
  public void initialize() {
    ...
    DataLogUtil.writeMessage(
        "DriveResetPose: Init, x =", curX, ", y =", curY, ", angle =", curAngle);
